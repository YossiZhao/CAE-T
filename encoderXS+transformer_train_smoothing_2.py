...
from models.encoder2 import res_encoderXS, res_encoderM
...
#         curr_lr_rt = poly_lr_scheduler(optimizer, init_lr=Configs['optimizer']['init_lr'], iter=epoch, max_iter=epochs)
        curr_lr_rt = Configs['optimizer']['init_lr']
        for batch_index, (data,target,_) in enumerate(train_loader, 0):
            optimizer.zero_grad()
    #     for batch_index, data in enumerate(train_loader, 0):
            data, target = data.to('cuda'), target.to('cuda')
...
if __name__ == "__main__":
    ...
    args = parser.parse_args(args=['configs/encoderXS+transformer.yml'])
